...
**Design:** Explicit `NoStateError` exceptions thrown mid-execution when accessing external shard state (Section 1, Step 4)

**Implementation:** Tracer-based detection via `CrossShardTracer.OnEnter()` in `orchestrator/statedb.go`

**How it works:**
- `CrossShardTracer` hooks into EVM's `OnEnter` callback to capture all CALL/DELEGATECALL/STATICCALL operations
- External calls (to addresses on different shards) are recorded as "pending external calls"
- After EVM execution completes, `HasPendingExternalCalls()` detects cross-shard access
- For each pending call, `RwSetRequest` is sent to the target shard's `/rw-set` endpoint
...
See `orchestrator/simulator.go:runSimulation()` for the iterative re-execution loop.

### Merkle Proofs (V2.3)

**Status:** Deferred (documented in `docs/TODO.md`)

The `ReadSetItem.Proof` field exists but is always empty. Full Merkle proof validation requires light client infrastructure and state proof generation which is not yet implemented. This is tracked as a future enhancement.
