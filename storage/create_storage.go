...
func main() {
	...
	err = GenerateHotelAddresses()
	if err != nil {
		panic(err)
	}

	err = GenerateTrainAddresses()
	if err != nil {
		panic(err)
	}

	err = GenerateTravelAddresses()
	if err != nil {
		panic(err)
	}
...
func GenerateHotelAddresses() error {
	return generateContractAddresses("./storage/hotelAddress.txt", "hotel")
}

func GenerateTrainAddresses() error {
	return generateContractAddresses("./storage/trainAddress.txt", "train")
}

func GenerateTravelAddresses() error {
	return generateContractAddresses("./storage/travelAddress.txt", "travel")
}

func generateContractAddresses(filename, contractType string) error {
	...
	for i := 0; i < numContracts; i++ {
		seed := fmt.Sprintf("%s-contract-%d", contractType, i)
		hash := sha256.Sum256([]byte(seed))
		addr := common.BytesToAddress(hash[:])
		if _, err := fmt.Fprintln(file, addr.Hex()); err != nil {
			return err
...
func CreateStorage(shardID int) {
	...
	// 같은 샤드 travel(local), 다른 샤드 travel(CT)
	// 가시적으로 address 관계 확인할 수 있게.
	// 0x(계약 식별자)(로컬)(읽기 쉬운 8bits)
...
func setContractAccounts(evm *vm.EVM, stateDB *state.StateDB, deployer common.Address, addresses []*common.Address, numShards, shardID int, contractType string) {
	...
	for _, address := range addresses {
		if int(address[len(address)-1])%numShards != shardID {
			continue
		}
		...
		if contractType == "travel" {
			...
			// Get deploy code with constructor arguments (train and hotel addresses)
			trainAddrHex := allTrainAddrs[index].Hex()[2:] // Remove "0x" prefix
			hotelAddrHex := allHotelAddrs[index].Hex()[2:]
			deployCode = GetDeployCode("travel", []byte(common.Bytes2Hex(creationBytecode)), trainAddrHex, hotelAddrHex)
		} else {
...
func GetDeployCode(contractName string, code []byte, arg1 string, arg2 string) []byte {
	switch contractName {
	case "train":
		// Train: 생성자 인자 없음 → 바이트코드만 반환
		return hexutil.MustDecode("0x" + string(code))
	case "hotel":
		// Hotel: 생성자 인자 없음 → 바이트코드만 반환
		return hexutil.MustDecode("0x" + string(code))
	case "travel":
		// Travel: 생성자가 (address train, address hotel) 받음
		// ABI 인코딩: 주소는 32바이트(64 hex)로 패딩 필요
		// "000000000000000000000000" = 24개 0 (12바이트)
		// + 주소 40 hex (20바이트) = 64 hex (32바이트)
		return hexutil.MustDecode("0x" + string(code) +
